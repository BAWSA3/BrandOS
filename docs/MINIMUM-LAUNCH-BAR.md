...
| `/api/x-tweets` cost-abuse hole (unauth, arbitrary handle, paid data) | ✅ mitigated — IP rate-limit + cache + internal-token bypass (commit `4600ac5`). `INTERNAL_API_SECRET` set in both Vercel projects (prod + staging, all envs) 2026-06-18 — live on next deploy |
| `score-schemas.ts` (output validation) | ✅ done (commit `f280e7e`) — Zod validate + clamp 0-100 on x-brand-score / -enhanced / audit; heuristic fallback (brand score) or error (paid audit). No untyped JSON.parse of model output remains. Test: `npm run test:safety` (21/21) |
| `prompt-safety.ts` (injection guard) | ✅ done (commit `f280e7e`) — `wrapUntrusted` + `GUARD_PREAMBLE` + `INPUT_CAPS` applied to both score prompts + audit prompt. Behavioral live-resist test left manual |
