...
body {
  font-family:
    -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'SF Pro Text', 'Inter', 'Segoe UI', Roboto,
...
.spring-default {
  transition:
    transform var(--spring-default),
    opacity var(--spring-default);
}
.spring-bouncy {
  transition:
...
.spring-gentle {
  transition:
    transform var(--spring-gentle),
    opacity var(--spring-gentle);
}
```
