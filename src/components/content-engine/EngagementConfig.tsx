...
export default function EngagementConfig({ config, onSave }: EngagementConfigProps) {
  ...
  const update = (field: keyof EngagementContext, value: string) => {
    setDraft((prev) => ({
      ...prev,
      [field]:
        field === 'topGaps' || field === 'targetDate'
          ? value
          : value === ''
            ? undefined
            : Number(value),
  ...
  const handleSave = () => {
    onSave({
      ...
      engagement: {
        ...draft,
        topGaps:
          typeof draft.topGaps === 'string'
            ? (draft.topGaps as unknown as string)
                .split(',')
                .map((s) => s.trim())
                .filter(Boolean)
            : draft.topGaps,
            ...
            {eng.followerCount && (
              <span className="label-mono text-[var(--text-quaternary)]">
                ...
                <span className="text-[var(--text-primary)]">
                  {eng.followerCount.toLocaleString()}
                </span>
              </span>
            )}
            {eng.followerTarget && (
              <span className="label-mono text-[var(--text-quaternary)]">
                Target:{' '}
                <span className="text-[var(--text-primary)]">
                  {eng.followerTarget.toLocaleString()}
                </span>
              </span>
            )}
            {eng.engagementRate && (
              <span className="label-mono text-[var(--text-quaternary)]">
                Eng Rate: <span className="text-[var(--text-primary)]">{eng.engagementRate}%</span>
              </span>
            )}
            {eng.topGaps.length > 0 && (
              <span className="label-mono text-[var(--text-quaternary)]">
                Gaps: <span className="text-[var(--warning)]">{eng.topGaps.join(', ')}</span>
              </span>
            )}
          </div>
        ) : (
          <p className="font-mono text-xs text-[var(--text-quaternary)]">
            Add your metrics and voice settings to improve generation quality.
          </p>
        )}
      </div>
    );
    ...
    <div className="glass-card border border-[var(--surface-tertiary)] rounded-[var(--radius-md)] p-5 mb-4">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <Field
          ...
          value={draft.followerTarget ?? ''}
          onChange={(v) => update('followerTarget', v)}
          placeholder="e.g. 50000"
        />
        <Field
          label="Target Date"
          value={draft.targetDate ?? ''}
          onChange={(v) => setDraft((prev) => ({ ...prev, targetDate: v || undefined }))}
        ...
        <Field
          label="Avg Retweets"
          value={draft.avgRetweets ?? ''}
          onChange={(v) => update('avgRetweets', v)}
          placeholder="e.g. 1.15"
        />
        <Field
          label="CTA Score (0-100)"
          value={draft.ctaScore ?? ''}
          onChange={(v) => update('ctaScore', v)}
          placeholder="e.g. 45"
        />
        <Field
          label="CTA Target"
          value={draft.ctaTarget ?? ''}
          onChange={(v) => update('ctaTarget', v)}
          placeholder="e.g. 65"
        />
        <Field
          label="Top Gaps (comma-separated)"
          value={Array.isArray(draft.topGaps) ? draft.topGaps.join(', ') : ''}
          onChange={(v) =>
            setDraft((prev) => ({
              ...prev,
              topGaps: v
          ...
          placeholder="e.g. CTA, Hook"
        />
      </div>
      <div className="space-y-3 mb-4">
        <div>
          <label className="label-mono text-[var(--text-quaternary)] block mb-1">
            Voice Constraints (comma-separated)
          </label>
          <input
            value={voiceConstraints}
            onChange={(e) => setVoiceConstraints(e.target.value)}
            placeholder="e.g. lowercase, no em dashes, 0-2 emojis"
        ...
        <div>
          <label className="label-mono text-[var(--text-quaternary)] block mb-1">
            Never Say (comma-separated phrases)
          </label>
          <input
            value={neverSay}
            onChange={(e) => setNeverSay(e.target.value)}
            placeholder="e.g. let's dive in, game-changer, here's the thing"
            className="terminal-input w-full bg-[var(--surface)] border border-[var(--surface-tertiary)] rounded-[var(--radius-sm)] px-3 py-2 text-[var(--text-primary)] text-xs outline-none focus:border-[var(--accent)]"
          />
        </div>
        <div>
          <label className="label-mono text-[var(--text-quaternary)] block mb-1">
            Quick Topic Themes (comma-separated)
          </label>
          <input
            value={themes}
            onChange={(e) => setThemes(e.target.value)}
            placeholder="e.g. build updates, content systems, creator economy"
            className="terminal-input w-full bg-[var(--surface)] border border-[var(--surface-tertiary)] rounded-[var(--radius-sm)] px-3 py-2 text-[var(--text-primary)] text-xs outline-none focus:border-[var(--accent)]"
          />
        </div>
      </div>
      <div className="flex gap-2">
        <button
          onClick={handleSave}
          className="btn-primary px-4 py-2 text-xs font-medium rounded-[var(--radius-sm)]"
        >
          Save
        </button>
        <button
          onClick={() => setEditing(false)}
          className="label-mono text-[var(--text-quaternary)] border border-[var(--surface-tertiary)] px-4 py-2 rounded-[var(--radius-sm)] hover:text-[var(--text-secondary)] transition-[var(--transition-fast)]"
        >
          Cancel
        </button>
      </div>
