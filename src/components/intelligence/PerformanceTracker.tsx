  ...
  async function fetchPerformance() {
    ...
    try {
      const res = await fetch(
        `/api/content-intelligence/performance?brandId=${brandId}&window=${window}`
      );
      if (res.ok) {
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <div className="flex items-center gap-1">
                <p style={{ fontSize: 10, color: 'var(--text-tertiary)', marginBottom: 2 }}>
                  Eng. Rate
                </p>
                <Sparkline values={engagementHistory} color="#30D158" />
              </div>
              <p style={{ fontSize: 22, fontWeight: 700, color: '#30D158' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 22, fontWeight: 700, color: 'var(--text-primary)' }}>
                {formatNumber(data.avgImpressions)}
              </p>
            </div>
            <div>
              <p style={{ fontSize: 10, color: 'var(--text-tertiary)', marginBottom: 2 }}>
