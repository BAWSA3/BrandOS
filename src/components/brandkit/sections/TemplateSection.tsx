...
const templateTypes = [
  {
    id: 'social',
    label: 'Social Media',
    platforms: ['Instagram Post', 'Instagram Story', 'Twitter', 'LinkedIn', 'Facebook'],
  },
  ...
  { id: 'ad', label: 'Advertising', platforms: ['Banner', 'Leaderboard', 'Skyscraper', 'Square'] },
  {
    id: 'presentation',
    label: 'Presentation',
    platforms: ['Title Slide', 'Content Slide', 'Quote Slide'],
  },
...
export default function TemplateSection({ brandId }: TemplateSectionProps) {
  ...
  const handleCreateTemplate = () => {
    ...
    const type =
      templateTypes.find((t) => t.platforms.includes(selectedPlatform as never))?.id || 'social';
  ...
  const templatesByType = brandKit.templates.reduce(
    (acc, template) => {
        ...
        {templateTypes.map((type) => (
          <button
            ...
            className={`px-4 py-2 text-sm rounded-lg transition-colors ${
              selectedType === type.id ? 'bg-foreground text-background' : 'hover:bg-surface'
            }`}
          >
            {type.label}
              ...
              >
                ...
                {currentType?.platforms.map((platform) => (
                  <option key={platform} value={platform}>
                    {platform} ({templateDimensions[platform]?.width}×
                    {templateDimensions[platform]?.height})
                  </option>
                ))}
              </select>
            </div>
      ...
      {(templatesByType[selectedType]?.length || 0) === 0 ? (
        <div className="text-center py-12 border-2 border-dashed border-border rounded-xl">
          ...
          <p className="text-xs text-muted">
            Create templates to maintain consistent brand layouts
          </p>
        </div>
      ) : (
