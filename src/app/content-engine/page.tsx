...
const FORMAT_OPTIONS = ['--', 'Thought', 'Hot Take', 'Story', 'Conversational'];

const CTA_OPTIONS = ['--', 'Bookmark This', 'Agree/Disagree', 'Quote RT'];
      ...
      {(open || closing) && (
        ...
        >
          ...
          <div
            ...
            style={{
              boxShadow: '0 2px 12px rgba(0, 71, 255, 0.08)',
              scrollbarWidth: 'none',
              minWidth: 120,
...
export default function ContentEnginePage() {
  ...
  const [formatBySlot, setFormatBySlot] = useState<Record<SlotId, string>>({
    post1: '--',
    post2: '--',
  });
  ...
  const config = {
    ...DEFAULT_CONTENT_ENGINE_CONFIG,
    ...rawConfig,
    doPatterns: rawConfig.doPatterns || [],
  };
  ...
  const generate = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Generation failed. Check connection and try again.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div
      className={`min-h-screen bg-[var(--background)] text-[var(--text-primary)] relative ${!config.scannedHandle ? 'flex items-center justify-center' : ''}`}
    >
      {/* Grid overlay */}
      <div
      ...
      <div
        className={`max-w-[680px] mx-auto px-6 relative ${config.scannedHandle ? 'py-12' : 'py-6 w-full'}`}
      >
        {/* Header */}
        ...
        >
          ...
          <div className="glass-card border border-[var(--surface-tertiary)] rounded-[var(--radius-md)] p-6 mb-4">
                ...
                <div className="flex-1 flex items-center gap-1.5">
                  ...
                  {posts.includes('post1') && posts.length > 1 && (
                    <button
                      ...
                      title={
                        confirmingDelete === 'post1' ? 'Click again to confirm' : 'Remove post'
                      }
                    >
                      {confirmingDelete === 'post1' ? (
                        <span
                ...
                {posts.includes('post2') ? (
                  ...
                  >
                    ...
                    <button
                      ...
                      title={
                        confirmingDelete === 'post2' ? 'Click again to confirm' : 'Remove post'
                      }
                    >
                      {confirmingDelete === 'post2' ? (
                        <span
            ...
            <div
              className="mb-5 px-4 py-3 rounded-[var(--radius-sm)] border border-[var(--surface-tertiary)]"
            ...
            {atLimit ? (
              ...
              <div
                className="border rounded-[var(--radius-sm)] p-4 text-center"
                style={{ borderColor: 'var(--accent)', background: 'rgba(0, 71, 255, 0.04)' }}
              >
                <p className="font-mono text-xs text-[var(--text-primary)] mb-1 font-bold">
                  you've used all {generationLimit} generations
                </p>
            ...
            {!isUnlocked && !atLimit && remaining <= 2 && (
              <div
                className="mt-3 border rounded-[var(--radius-sm)] px-4 py-3"
                style={{
              ...
              >
                <div className="flex items-center justify-between">
                  <p className="font-mono text-[10px] text-[var(--text-quaternary)]">
                    {remaining} generation{remaining !== 1 ? 's' : ''} left. invite a friend to
                    unlock unlimited.
                  </p>
                  <button
                    onClick={copyInviteLink}
            ...
            <div className="glass-card border border-[var(--surface-tertiary)] rounded-[var(--radius-md)] overflow-hidden">
              ...
              <div className="border-t border-[var(--surface-tertiary)] px-5 py-2.5">
                <p className="font-mono text-[11px] text-[var(--text-quaternary)]">
                  ...
                  <Link href="/early-access" className="text-[var(--accent)] underline">
