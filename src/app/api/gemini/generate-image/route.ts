...
export async function POST(request: Request) {
  try {
    ...
    const { type, brandName, style, description, purpose, platform, message } =
      await request.json();
    ...
    switch (type) {
      case 'logo':
        prompt = brandPrompts.logo(
          brandName || 'Brand',
          style || 'modern minimal',
          description || 'A professional brand logo'
        );
        break;
      case 'imagery':
        prompt = brandPrompts.brandImagery(
          brandName || 'Brand',
          style || 'modern',
          description || 'Brand imagery'
        );
        break;
      case 'icon':
        ...
        break;
      case 'social':
        prompt = brandPrompts.socialGraphic(
          brandName || 'Brand',
          platform || 'Instagram',
          message || 'Brand announcement'
        );
        break;
      default:
    ...
    return NextResponse.json({ error: 'No image data in response' }, { status: 400 });
  } catch (error) {
