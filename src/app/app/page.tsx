...
function HomeContent() {
  ...
  const [activeTab, setActiveTab] = useState<SubTab>(
    getDefaultTabForPhase(phaseProgress.lastActivePhase || 'home')
  );
  ...
  const [showPhasesBreakdown, setShowPhasesBreakdown] = useState(
    showPhasesParam && !phaseProgress.hasCompletedOnboarding
  );
    ...
    if (extractedBrand.colors) {
      setBrandDNA({
        colors: {
          ...
          secondary:
            extractedBrand.colors.secondary?.value || brandDNA?.colors?.secondary || '#F2F0EF',
  ...
  const dashboardStats = {
    ...
    averageScore:
      history.filter((h) => h.type === 'check').length > 0
        ? Math.round(
            history
  ...
  const contentTypes = Object.entries(contentTypeLabels) as [
    ContentType,
    (typeof contentTypeLabels)[string],
  ][];

  // Show phases breakdown for new users coming from landing page
  if (showPhasesBreakdown) {
    return <PhasesBreakdown onGetStarted={handlePhasesGetStarted} onSkip={handlePhasesSkip} />;
  }

  // Show import hub for new users (first step) or when explicitly requested
  if (
    showImportHub ||
    (!phaseProgress.hasCompletedOnboarding && !showOnboarding && !showPhasesBreakdown)
  ) {
    return (
      <BrandImportHub onStartFresh={handleStartFresh} onImportComplete={handleImportComplete} />
    );
  }
  ...
  return (
    ...
    >
        ...
        <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 animate-fade-in">
          ...
          >
            ...
            <span style={{ fontSize: 14, color: 'var(--text-primary)' }}>
              Brand DNA imported successfully
            </span>
            <button
              onClick={() => {
                setShowImportedWelcome(false);
            ...
            <div className="p-1.5 max-h-48 overflow-y-auto">
              {brands.map((brand) => (
                <div
                  ...
                  style={{
                    background:
                      brand.id === currentBrandId ? 'rgba(10,132,255,0.1)' : 'transparent',
                  }}
                  onClick={() => {
                ...
                >
                  <span
                    style={{
                      fontSize: 13,
                      color: brand.id === currentBrandId ? 'var(--accent)' : 'var(--text-primary)',
                    }}
                  >
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        {activeTab === 'brand' && (
          <div className="animate-fade-in">
            ...
            <section className="max-w-2xl mx-auto px-6 py-16">
              ...
              {/* Brand Colors */}
              <div className="mb-16">
                <h3 className="text-xs uppercase tracking-widest text-muted mb-8">Brand Colors</h3>
              ...
              {/* Tone Spectrum */}
              <div className="mb-16">
                <h3 className="text-xs uppercase tracking-widest text-muted mb-8">Tone Spectrum</h3>
              ...
              <div className="mb-16">
                ...
                <div className="space-y-3 mt-4">
                  {brandDNA?.voiceSamples?.map((sample, i) => (
                    <div
                      key={i}
                      className="group relative p-3 bg-surface rounded-lg cursor-pointer hover:bg-border transition-colors"
                      onClick={() => removeVoiceSample(i)}
                    >
                      <p className="text-sm pr-6 italic">&ldquo;{sample}&rdquo;</p>
                      <span className="absolute top-2 right-2 text-muted opacity-0 group-hover:opacity-100">
                        ×
                      </span>
                    </div>
                  ))}
              ...
              {brandCompleteness >= 30 && (
                <div className="p-6 bg-surface rounded-xl text-center">
                  <p className="text-sm text-muted mb-4">
                    Your brand is {brandCompleteness}% complete. Ready to check some content?
                  </p>
                  <button
                ...
                <div className="mb-8 p-4 bg-surface rounded-lg animate-fade-in">
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-4">
                    {[
                      {
                        label: 'Formality',
                        value: toneAnalysis.formality,
                        target: brandDNA?.tone?.minimal || 50,
                      },
                      {
                        label: 'Energy',
                        value: toneAnalysis.energy,
                        target: brandDNA?.tone?.playful || 50,
                      },
                      {
                        label: 'Confidence',
                        value: toneAnalysis.confidence,
                        target: brandDNA?.tone?.bold || 50,
                      },
                      {
                        label: 'Style',
                        value: toneAnalysis.style,
                        target: brandDNA?.tone?.experimental || 50,
                      },
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 8 }}>
                          ...
                          <span
                            style={{
                              ...
                              borderRadius: 8,
                              background:
                                authenticityScore.verdict === 'authentic' ||
                                authenticityScore.verdict === 'mostly_authentic'
                                  ? 'rgba(52, 199, 89, 0.15)'
                    ...
                    {checkResult.strengths?.length > 0 && (
                      <div>
                        ...
                        {checkResult.strengths.map((s, i) => (
                          <p key={i} className="text-sm py-2 border-b border-border">
                            {s}
                          </p>
                        ))}
                      </div>
        ...
        {activeTab === 'platforms' && (
          <div className="animate-fade-in">
            <section className="pb-8">
              ...
              <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginBottom: 24 }}>
                Adapt content for each platform while preserving brand identity.
              </p>
            </section>
            <section className="max-w-3xl mx-auto">
              <PlatformAdapter />
              ...
              {dashboardStats.totalChecks === 0 && dashboardStats.totalGenerations === 0 && (
                <div className="text-center py-16">
                  <p className="text-muted mb-4">
                    No activity yet. Start by checking or generating content.
                  </p>
                  <button
                    onClick={() => handlePhaseChange('check')}
                    className="px-6 py-3 bg-foreground text-background rounded-full text-sm font-medium hover:opacity-90 transition-opacity"
        ...
        {activeTab === 'history' && (
          <div className="animate-fade-in">
            <section className="pb-8">
              ...
              <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginBottom: 24 }}>
                Review your previous checks and generations.
              </p>
            </section>
                      ...
                      <div key={item.id} className="p-4 bg-surface rounded-lg">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {item.contentType && item.contentType !== 'general' && (
                              <span className="text-xs text-muted">
                                {contentTypeLabels[item.contentType]?.label}
                              </span>
                            )}
                          </div>
                        ...
                        {item.type === 'check' && (
                          <div className="text-xl font-light">
                            {(item.output as CheckResult).score}
        ...
        {activeTab === 'export' && (
          <div className="animate-fade-in">
            <section className="pb-8">
              ...
              <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginBottom: 24 }}>
                Download your brand guidelines.
              </p>
            </section>
            ...
            <section className="max-w-2xl mx-auto">
              <div className="grid grid-cols-3 gap-4 mb-8">
                <button
                  ...
                  className="py-4 border border-border rounded-lg hover:border-foreground transition-colors"
                >
                ...
                <button
                  onClick={handleShare}
                  disabled={isSharing}
                  className="py-4 border border-border rounded-lg hover:border-foreground transition-colors disabled:opacity-50"
                >
              ...
              {shareUrl && (
                <div className="mb-16 p-4 bg-surface rounded-lg animate-fade-in">
                  ...
                  <p className="text-xs text-muted mt-2">
                    Anyone with this link can view and import your brand guidelines.
                  </p>
                </div>
              )}
              ...
              <div
                ref={exportRef}
                className="border border-border rounded-lg p-8 print:border-none"
              >
              ...
              <div className="mb-8">
                ...
                <div className="space-y-2 mt-4">
                  {competitorSamples.map((sample, i) => (
                    <div
                      key={i}
                      className="group flex items-start gap-2 p-3 bg-surface rounded-lg cursor-pointer"
                      onClick={() => removeCompetitorSample(i)}
                    >
              ...
              {competitorAnalysis && (
                <div className="mt-16 animate-fade-in">
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      {[
                        {
                          label: 'Formality',
                          yours: brandDNA?.tone?.minimal || 50,
                          theirs: competitorAnalysis.competitorTone.formality,
                        },
                        {
                          label: 'Energy',
                          yours: brandDNA?.tone?.playful || 50,
                          theirs: competitorAnalysis.competitorTone.energy,
                        },
                        {
                          label: 'Confidence',
                          yours: brandDNA?.tone?.bold || 50,
                          theirs: competitorAnalysis.competitorTone.confidence,
                        },
                        {
                          label: 'Style',
                          yours: brandDNA?.tone?.experimental || 50,
                          theirs: competitorAnalysis.competitorTone.style,
                        },
                  ...
                  <div className="mt-8">
                    ...
                    <p className="text-sm p-4 bg-surface rounded-lg">
                      {competitorAnalysis.uniquePositioning}
                    </p>
                  </div>
                </div>
              )}
        ...
        <footer
          className="py-10 mt-12 print:hidden"
