...
var gsap,
  ...
  _sync = function _sync() {
    return (
      _getTime() - _lastScrollTime > 34 && (_rafID || (_rafID = requestAnimationFrame(_updateAll)))
    );
  },
  ...
  _parsePosition = function _parsePosition(
    ...
    if (marker) {
      ...
      _positionMarker(
        ...
        (isStart && position > 20) ||
          (!isStart &&
            (useFixedPosition ? Math.max(_body[p1], _docEl[p1]) : marker.parentNode[p1]) <=
              position + 1)
      );
    ...
    self.refresh = function (soft, force, position, pinOffset) {
      ...
      while (i--) {
        ...
        if (
          ...
          cs =
            curTrigger.end - (self._startClamp ? Math.max(0, curTrigger.start) : curTrigger.start);
      ...
      if (pin && !(_clampingMax && self.end >= _maxScroll(scroller, direction))) {
        ...
        if (!max && end > 1) {
          ...
          if (
            isViewport &&
            _getComputedStyle(_body)['overflow' + direction.a.toUpperCase()] !== 'scroll'
          ) {
      ...
      animation &&
        ...
        animation.progress(prevAnimProgress || 0, true).render(animation.time(), true, true); // must force a re-render because if saveStyles() was used on the target(s), the styles could have been wiped out during the refresh().
      ...
      if (
        ...
        animation &&
          !isToggle &&
          (animation._initted || prevProgress || animation.vars.immediateRender !== false) &&
        ...
        self.progress =
          isFirstRefresh || (scroll1 - start) / change === prevProgress ? 0 : prevProgress;
      }
      ...
      if (clipped !== prevProgress && self.enabled) {
        ...
        if (stateChanged && !_refreshing) {
          ...
          if (
            fastScrollEnd &&
            !isActive &&
            Math.abs(self.getVelocity()) > (_isNumber(fastScrollEnd) ? fastScrollEnd : 2500)
          ) {
            _endAnimation(self.callbackAnimation);
    ...
    func ||
      function (a, b) {
        return (
          ...
          ((b.vars.containerAnimation ? 1e6 : b._sortY) + (b.vars.refreshPriority || 0) * -1e6)
        );
      }
