...
var gsap,
  ...
  _getTarget = function _getTarget(t, self) {
    return (
      ((self && self._ctx && self._ctx.selector) || gsap.utils.toArray)(t)[0] ||
  ...
  _initCore = function _initCore(core) {
    ...
    if (!_coreInitted && gsap && typeof document !== 'undefined' && document.body) {
      ...
      _isTouch = Observer.isTouch =
        _win.matchMedia && _win.matchMedia('(hover: none), (pointer: coarse)').matches
          ? 1
          : 'ontouchstart' in _win || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0
            ? 2
            : 0;
...
export var Observer = /*#__PURE__*/ (function () {
  ...
  _proto.init = function init(vars) {
    ...
    var id,
      ...
      scrollY = scrollFuncY(),
      limitToTouch =
        ~type.indexOf('touch') && !~type.indexOf('pointer') && _eventTypes[0] === 'pointerdown',
      ...
      _onDrag = function _onDrag(e) {
        ...
        if (
          isDragging ||
          ((dx || dy) &&
            (Math.abs(self.startX - x) >= dragMinimum || Math.abs(self.startY - y) >= dragMinimum))
        ) {
      ...
      _onWheel = function _onWheel(e) {
        ...
        var multiplier =
          (e.deltaMode === 1 ? lineHeight : e.deltaMode === 2 ? _win.innerHeight : 1) * wheelSpeed;
    ...
    self.enable = function (e) {
      if (!self.isEnabled) {
        ...
        type.indexOf('scroll') >= 0 &&
          _addListener(isViewport ? ownerDoc : target, 'scroll', onScroll, passive, capture);
