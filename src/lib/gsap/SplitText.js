...
let gsap,
  ...
  _toArray = (r) =>
    typeof r === 'string'
      ? _toArray(document.querySelectorAll(r))
      : 'length' in r
        ? Array.from(r).reduce((acc, cur) => {
        ...
        while (--i > -1) {
          ...
          for (char of charsFound) {
            if (char.startsWith(word) && char.length > word.length) {
              ...
              combined = word;
              while (
                char.startsWith((combined += collection[i + ++slots])) &&
                combined.length < char.length
              ) {}
  ...
  _disallowInline = (element) =>
    window.getComputedStyle(element).display === 'inline' &&
    (element.style.display = 'inline-block'),
  _insertNodeBefore = (newChild, parent, existingChild) =>
    parent.insertBefore(
      ...
      if (curNode.nodeType === 3) {
        ...
        words =
          wordDelimString || wordDelimSplitter
            ? curTextContent.split(wordDelimSplitter || wordDelimString)
            : curTextContent.match(charSplitRegEx) || _emptyArray;
        ...
        for (j = 1; j <= words.length; j++) {
          ...
          if (!reduceWhiteSpace && wordText === '') {
            ...
            if (
              ...
              j === 1 &&
              !startsWithSpace &&
              wordsCollection.indexOf(ignoredPreviousSibling.parentNode) > -1
            ) {
            ...
            if (charWrapper) {
              curWordChars = _charSegmenter
                ? _stretchToFitSpecialChars(
                    [..._charSegmenter.segment(wordText)].map((s) => s.segment),
                    specialCharsRegEx
                  )
                : wordText.match(charSplitRegEx) || _emptyArray;
              for (k = 0; k < curWordChars.length; k++) {
                curWordEl.appendChild(
                  curWordChars[k] === ' '
                    ? document.createTextNode(' ')
                    : charWrapper(curWordChars[k])
                );
              }
            }
        ...
        if (ignore && ignore.indexOf(curNode) > -1) {
          wordsCollection.indexOf(curNode.previousSibling) > -1 &&
            wordsCollection[wordsCollection.length - 1].appendChild(curNode);
  ...
  constructor(elements, config) {
    ...
    this.elements.forEach((el) => {
      ...
      config.overwrite !== false &&
        ((_a = el[_splitProp]) == null
          ? void 0
          : _a._data.orig.filter(({ element }) => element === el).forEach(_revertOriginal));
      el[_splitProp] = this;
    });
    ...
    (this._ctx || _defaultContext).add(() => {
      ...
      let {
        ...
        specialCharsRegEx =
          ...
          ('push' in specialChars
            ...
            : specialChars),
        finalCharSplitRegEx = specialCharsRegEx
      ...
      if (splitCharacters || splitWords || splitLines) {
        this.elements.forEach((element, index) => {
          ...
          aria === 'auto'
            ...
            : aria === 'hidden' && element.setAttribute('aria-hidden', 'true');
          let chars = [],
            words = [],
            lines = [],
          ...
          if (splitLines) {
            let nodes = _toArray(element.childNodes),
              ...
              toRemove = [],
              lineStartIndex = 0,
              allBounds = nodes.map((n) =>
                n.nodeType === 1 ? n.getBoundingClientRect() : _emptyBounds
              ),
      ...
      _fonts &&
        ...
        (autoSplit
          ...
          : _fonts.status === 'loading' && console.warn('SplitText called before fonts loaded'));
