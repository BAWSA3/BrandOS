...
var _id = 1,
  ...
  _fit = function _fit(fromState, toState, scale, applyProps, fitChild, vars) {
    ...
    if (scale) {
      scaleX *=
        width === dimensionState.width || !dimensionState.width ? 1 : width / dimensionState.width; // note if widths are both 0, we should make scaleX 1 - some elements have box-sizing that incorporates padding, etc. and we don't want it to collapse in that case.
  ...
  _fromTo = function _fromTo(fromState, toState, vars, relative) {
    ...
    for (p in toState.idLookup) {
      ...
      if (fromNode) {
        ...
        if (comp.sd) {
          ...
          interrupted &&
            _recordInlineStyles(comp.b, props ? _memoizedRemoveProps[props] : _removeProps);
          fade &&
            comps.push(
              (comp.swap = {
                t: fromNode.element,
    ...
    if (_batch) {
      ...
      absolute &&
        (_batch$_abs = _batch._abs).push.apply(_batch$_abs, _filterComps(comps, absolute));
...
var FlipState = /*#__PURE__*/ (function () {
  ...
  _proto.add = function add(state) {
    ...
    while (i--) {
      ...
      if (
        es2 &&
        (es.element === es2.element || (alt[es.id] && alt[es.id].element === es.element))
      ) {
  ...
  _proto.compare = function compare(state) {
    ...
    for (p in l1) {
      ...
      if (s2Alt) {
        ...
        c1 =
          s1Alt && !s1.isVisible && !s1Alt.isVisible && c2.element === s1Alt.element ? s1Alt : s1; //c1.element !== c2.element && c1.element === s2.element && (c2 = s2);
        ...
        s1Alt &&
          placeIfDoesNotExist(s1Alt, s2Alt.element === s1Alt.element ? s2Alt : s2, s1Alt.element);
      } else {
...
var ElementState = /*#__PURE__*/ (function () {
  ...
  _proto2.isDifferent = function isDifferent(state) {
    ...
    return (
      ...
      (this.props && state.props && JSON.stringify(this.props) !== JSON.stringify(state.props))
    );
  };
  ...
  _proto2.update = function update(props, simple) {
    ...
    self.ctm =
      element.getCTM && element.nodeName.toLowerCase() === 'svg' && _getCTM(element).inverse();
    self.simple =
      simple || (_round(m.a) === 1 && !_round(m.b) && !_round(m.c) && _round(m.d) === 1); // allows us to speed through some other tasks if it's not scale/rotated
