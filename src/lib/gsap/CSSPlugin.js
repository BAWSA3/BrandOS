  ...
  _removeProperty = function _removeProperty(target, property) {
    if (property) {
      ...
      if (style.removeProperty) {
        ...
        style.removeProperty(
          first2Chars === '--' ? property : property.replace(_capsExp, '-$1').toLowerCase()
        );
      } else {
        ...
        if (isTransformRelated) {
          ...
          if (p === 'scale') {
            this._pt = new PropTween(
              ...
              'scaleY',
              cache.scaleY,
              (relative ? _parseRelative(cache.scaleY, relative + endNum) : endNum) -
                cache.scaleY || 0,
              _renderCSSProp
            );
            this._pt.u = 0;
