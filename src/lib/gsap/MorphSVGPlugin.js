...
var gsap,
  ...
  _subdivideSegmentQty = function _subdivideSegmentQty(segment, quantity) {
    ...
    for (; i < l; i += 6) {
      distances.push(
        Math.pow(segment[i] - segment[i + 6], 2) + Math.pow(segment[i + 1] - segment[i + 7], 2)
      );
    }
  ...
  _smoothRawPath = function _smoothRawPath(rawPath, config) {
    ...
    for (j = 0; j < sortedRawPath.length; j++) {
      ...
      segmentPointsToAdd = Math.max(
        redraw ? 10 : 4,
        templateRawPath
          ? Math.round(
              templateRawPath[j]
                ? templateRawPath[j].length / 6
                : sortedRawPath[j].totalLength / anchorDistance || 0
            )
          : Math.round(
              (pointsAdded / points +
                (redraw
                  ? segment.totalLength / rawPath.totalLength
                  : segment.length / rawPath.totalPoints)) *
                points
            ) - pointsAdded
      );
      ...
      if (
        j >= maxSegments ||
        (templateRawPath && (!templateRawPath[j] || _segmentCanBeIgnored(templateRawPath[j])))
      ) {
        ...
        smoothSegment = pointsToSegment(
          segmentToDistributedPoints(segment, segmentPointsToAdd),
          config.curviness
        );
        segment.length = 0;
  ...
  _equalizeSegmentQuantity = function _equalizeSegmentQuantity(
    ...
    if (dif) {
      dif < 0 && (dif = -dif);
      longer[0].length > shorter[0].length &&
        _subdivideSegmentQty(shorter[0], ((longer[0].length - shorter[0].length) / 6) | 0); // since we use shorter[0] as the one to map the origination point of any brand new fabricated segments, do any subdividing first so that there are more points to choose from (if necessary)
  ...
  _pathFilter = function _pathFilter(a, shapeIndex, map, precompile, fillSafe) {
    ...
    (precompile === 'log' || precompile === true) &&
...
export var MorphSVGPlugin = {
  ...
  init: function init(target, value, tween, index, targets) {
    ...
    smooth === true || smooth === 'auto'
      ? (smooth = {})
      : typeof smooth === 'number' &&
        (smooth = {
          points: smooth,
        });
      ...
      if (p === 'd' || this._prop) {
        ...
        if (smooth) {
          ...
          _smoothRawPath(
            ...
            _extends({}, smooth, {
              points:
                +smooth.points ||
                Math.max(_getDefaultSmoothPoints(start), _getDefaultSmoothPoints(end)),
        ...
        while (--j > -1) {
          ...
          for (i = 0; i < l; i += 6) {
            if (endSeg[i] !== startSeg[i] || endSeg[i + 1] !== startSeg[i + 1]) {
              if (useRotation) {
                ...
                pt =
                  this.add(startSeg, i + 1, startSeg[i + 1], endSeg[i + 1], 0, 0, 0, 0, 0, 1) || pt;
              }
            }
          }
          ...
          for (i = 0; i < l; i += 2) {
            if (
              curveMode &&
              (startCPData[i] !== endCPData[i] || startCPData[i + 1] !== endCPData[i + 1]) &&
              startCPData[i + 1] &&
              endCPData[i + 1]
            ) {
              ...
              endSeg[i + 1] !== startSeg[i + 1] &&
                (pt =
                  this.add(startSeg, i + 1, startSeg[i + 1], endSeg[i + 1], 0, 0, 0, 0, 0, 1) ||
                  pt);
            }
          }
        }
