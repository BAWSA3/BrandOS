...
var _svgPathExp = /[achlmqstvz]|(-?\d*\.?\d*(?:e[\-+]?\d+)?)[0-9]/gi,
  ...
  _segmentIsClosed = function _segmentIsClosed(segment) {
    return (segment.closed =
      Math.abs(segment[0] - segment[segment.length - 2]) < 0.001 &&
      Math.abs(segment[1] - segment[segment.length - 1]) < 0.001);
  },
...
export function segmentToDistributedPoints(segment, totalPoints) {
  ...
  for (; i < l; i += 6) {
    if (
      Math.abs(
        _atan2(segment[i + 1] - segment[i - 1], segment[i] - segment[i - 2]) -
          _atan2(segment[i + 3] - segment[i + 1], segment[i + 2] - segment[i])
      ) > limit
    ) {
  ...
  if (totalPoints > l) {
    ...
    for (ci = 0; ci < l; ci++) {
      ...
      curvePoints =
        Math.round((samples[sampleIndex - 1] / totalLength) * totalPoints) - curvePointsCumulative;
      ...
      for (j = 1; j <= curvePoints; j++) {
        ...
        i = lookup.length
          ? lookup[length < totalLength ? ~~(length / segment.minLength) : lookup.length - 1] || 0
          : _getSampleIndex(samples, length, length / totalLength);
        min = i ? samples[i - 1] : 0;
        max = samples[i];
  ...
  if (
    ...
    Math.abs(
      _atan2(segment[i + 1] - segment[i - 1], segment[i] - segment[i - 2]) -
        _atan2(segment[3] - segment[1], segment[2] - segment[0])
    ) <= limit
  ) {
